...
void setup(){ 
  ...
  // este sera el masti y la textura
...
//y dependiendo de la dificultad que la persona elija sera el tiempo que tendra
...
void draw(){
  ...
  if (inicio == false && juegoIniciado == true) {
    ...
    textSize(80);
    textAlign(CENTER);
    //agregamos el texto que aparecera en nuestro juego dependiendo de la dificultad elejida
    fill(0);
  ...
  pushMatrix();
  translate(50,0);
  for (int i = 0 ; i < letrasIncorrectas.size() ; i++) { //esto nos va a ayudar a saber cuando las letras sean incorrectas
    fill(255,0,0);
  ...
  for (int i = 0 ; i < palabra.length() ; i++) {
    ...
    stroke(0);
    translate(50,600);
    presentacionPalabra();//para que aparescan las lineas debajo de las palabras correctas 
    popMatrix();
    ...
    if (oportunidad == 0) {
      ...
      gameWin = false;
      juegoIniciado = false;
    }// esta parte del codigo es muy importante ya que es la logica que se va a basar el juego
    pushMatrix();  
...
void ElegirPalabra() { // esto nos ayuda a elejir una palabra random de nuestro arreglo 
...
void matar(){ // esto hace que cuando nos equivoquemos vallan apareciendo nuestro personaje
  if (oportunidad == 5) {
    p.cabeza();
    popMatrix();
    ps.update();
...
// guarda la tecla.
void inputLogic() { // esto determina que tecla del teclado se va tocando
...
//cuando el jugador perdio mostrara todos los texto .
void perdiste() {
  ...
  text("[S]i? or [N]o? ", width / 2, 700);// si decide jugar volvera a la parte de setub y hara todo lo anterior pero si decide que no saldra del programa
  if ( key == 's' || key == 'S') {
    setup();
  } if( key == 'n' || key == 'N') {
    exit();
...
void ganar() {
  ...
  text("Tus puntos son: " + "" + acierto, width / 2, 500);
  text("Quieres volver a jugar? ", width / 2, 600 );
  text("[S]i? or [N]o? ", width / 2, 700);// y nos preguntara lo mismo
  if ( key == 's' || key == 'S') { 
    setup();
  } if( key == 'n' || key == 'N') {
    exit();
...
//esto lo pongo abajo para que las personas no vean las palabras 
